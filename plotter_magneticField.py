...
def calculate_B_field(R, Z):
    ...
    r_squared = R**2 + Z**2 + epsilon
    BR = PHYSICS_PARAMS["B0"] * PHYSICS_PARAMS["r0"] ** 2 * R / r_squared ** (3 / 2)
    BZ = PHYSICS_PARAMS["B0"] * PHYSICS_PARAMS["r0"] ** 2 * Z / r_squared ** (3 / 2)
    return BR, BZ
...
def create_plot_mag(plot_type, plot_func):
    """Create and save a plot with error handling."""
    print_section(f"Creating {str(plot_type)} plot")
    try:
        plot_func()
        print(
            f"{Fore.GREEN}✔ {str(plot_type)} plot created successfully{Style.RESET_ALL}"
        )
    except Exception as e:
        print(f"{Fore.RED}✘ Error creating {str(plot_type)} plot: {e}{Style.RESET_ALL}")
...
def create_3d_quiver_plot():
    ...
    Y = np.linspace(
        *PLOT_PARAMS["3d_quiver"]["Y_range"], PLOT_PARAMS["3d_quiver"]["grid_size"]
    )
    ...
    quiver = ax.quiver(
        ...
        BY,
        BZ,
        length=PLOT_PARAMS["3d_quiver"]["quiver_length"],
        normalize=True,
...
def create_3d_contour_plot():
    ...
    Y = np.linspace(
        *PLOT_PARAMS["3d_contour"]["Y_range"], PLOT_PARAMS["3d_contour"]["grid_size"]
    )
    R, Y, Z = np.meshgrid(R, Y, Z)
    ...
    contour = ax.contourf(
        R[:, :, 0],
        Y[:, :, 0],
        B_mag[:, :, PLOT_PARAMS["3d_contour"]["grid_size"] // 2],
        levels=PLOT_PARAMS["3d_contour"]["contour_levels"],
        cmap="viridis",
        zdir="z",
...
def create_2d_quiver_plot():
    ...
    singularity_threshold = PLOT_PARAMS["2d_quiver"].get(
    ...
    mask = (
        R**2 + Z**2
    ) > singularity_threshold**2  # Exclude points too close to the singularity
    R_filtered = R[mask]
    Z_filtered = Z[mask]
    BR_filtered = BR[mask]
    BZ_filtered = BZ[mask]
    ...
    cbar = plt.colorbar(
        quiver, label="Magnetic Field Strength (T)", pad=PLOT_CONFIG["colorbar_pad"]
    )
    cbar.ax.tick_params(labelsize=PLOT_CONFIG["tick_label_size"])
...
if __name__ == "__main__":
    ...
    create_plot_mag("2D Streamplot", create_2d_streamplot)
    create_plot_mag("3D Quiver Plot", create_3d_quiver_plot)
    create_plot_mag("2D Contour Plot", create_2d_contour_plot)
    create_plot_mag("3D Contour Plot", create_3d_contour_plot)
    create_plot_mag("2D Quiver Plot", create_2d_quiver_plot)
