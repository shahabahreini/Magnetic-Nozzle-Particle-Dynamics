...
PLOT_CONFIG = {
    ...
    'output_folder': os.path.join('plots', 'BField_Plots'),
...
PLOT_PARAMS = {
    ...
    '2d_contour': {
        'grid_size': 20,        # Number of grid points in each dimension
        'R_range': (-2, 2),      # Range of R values (min, max)
        'Z_range': (-2, 2),      # Range of Z values (min, max)
        'contour_levels': 300,    # Number of contour levels
    },
    '3d_contour': {
    ...
    '2d_quiver': {
        ...
        'R_range': (-2, 2),              # Range of R values (min, max)
        'Z_range': (-2, 2),              # Range of Z values (min, max)
        'scale': 50,                     # Scale for quiver plot
        'headwidth': 3,                  # Width of arrow head
        'headlength': 3,                 # Length of arrow head
        'singularity_threshold': 0,    # Threshold to avoid plotting near singularity
    },
}
...
def create_directory(path):
    ...
    try:
        ...
        print(f"{Fore.RED}✘ Error creating directory {
...
def calculate_B_field(R, Z):
    r = np.sqrt(R**2 + Z**2)
    BR = PHYSICS_PARAMS['B0'] * \
        PHYSICS_PARAMS['r0']**2 * R / (R**2 + Z**2)**(3/2)
    BZ = PHYSICS_PARAMS['B0'] * \
        PHYSICS_PARAMS['r0']**2 * Z / (R**2 + Z**2)**(3/2)
    return BR, BZ
...
def create_plot(plot_type, plot_func):
    ...
    try:
        plot_func()
        print(f"{Fore.GREEN}✔ {plot_type} plot created successfully{
              Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}✘ Error creating {
...
def create_2d_streamplot():
    ...
    R = np.linspace(*PLOT_PARAMS['2d_streamplot']['R_range'],
                    PLOT_PARAMS['2d_streamplot']['grid_size'])
    Z = np.linspace(*PLOT_PARAMS['2d_streamplot']['Z_range'],
                    PLOT_PARAMS['2d_streamplot']['grid_size'])
    R, Z = np.meshgrid(R, Z)
    ...
    cbar = plt.colorbar(
        streamplot.lines, label='Magnetic Field Strength (T)', pad=PLOT_CONFIG['colorbar_pad'])
...
def create_3d_quiver_plot():
    ...
    R = np.linspace(*PLOT_PARAMS['3d_quiver']['R_range'],
                    PLOT_PARAMS['3d_quiver']['grid_size'])
    Z = np.linspace(*PLOT_PARAMS['3d_quiver']['Z_range'],
                    PLOT_PARAMS['3d_quiver']['grid_size'])
    Y = np.linspace(*PLOT_PARAMS['3d_quiver']['Y_range'],
                    PLOT_PARAMS['3d_quiver']['grid_size'])
    ...
    sm.set_array([])
    cbar = fig.colorbar(
        sm, ax=ax, label='Magnetic Field Strength (T)', pad=PLOT_CONFIG['colorbar_pad'])
    cbar.ax.tick_params(labelsize=PLOT_CONFIG['tick_label_size'])
...
def create_2d_contour_plot():
    ...
    R = np.linspace(*PLOT_PARAMS['2d_contour']['R_range'],
                    PLOT_PARAMS['2d_contour']['grid_size'])
    Z = np.linspace(*PLOT_PARAMS['2d_contour']['Z_range'],
                    PLOT_PARAMS['2d_contour']['grid_size'])
    R, Z = np.meshgrid(R, Z)
    ...
    contour = ax.contourf(
        R, Z, B_mag, levels=PLOT_PARAMS['2d_contour']['contour_levels'], cmap='viridis')
    cbar = plt.colorbar(
        contour, label='Magnetic Field Strength (T)', pad=PLOT_CONFIG['colorbar_pad'])
    cbar.ax.tick_params(labelsize=PLOT_CONFIG['tick_label_size'])
...
def create_3d_contour_plot():
    ...
    R = np.linspace(*PLOT_PARAMS['3d_contour']['R_range'],
                    PLOT_PARAMS['3d_contour']['grid_size'])
    Z = np.linspace(*PLOT_PARAMS['3d_contour']['Z_range'],
                    PLOT_PARAMS['3d_contour']['grid_size'])
    Y = np.linspace(*PLOT_PARAMS['3d_contour']['Y_range'],
                    PLOT_PARAMS['3d_contour']['grid_size'])
    R, Y, Z = np.meshgrid(R, Y, Z)
...
def create_2d_quiver_plot():
    ...
    singularity_threshold = PLOT_PARAMS['2d_quiver'].get('singularity_threshold', 0.1)  # Threshold to avoid near-zero values
    ...
    # Filter out points near the origin or below a threshold
    mask = (R**2 + Z**2) > singularity_threshold**2  # Exclude points too close to the singularity
    R_filtered = R[mask]
    Z_filtered = Z[mask]
    BR_filtered = BR[mask]
    BZ_filtered = BZ[mask]
    B_mag_filtered = B_mag[mask]

    # Quiver plot with 'tail' pivot to ensure arrows start from grid points
    quiver = ax.quiver(R_filtered, Z_filtered, BR_filtered, BZ_filtered, B_mag_filtered,
                       cmap='viridis', pivot='tail', scale=scale, headwidth=headwidth, headlength=headlength)
...
if __name__ == "__main__":
    ...
    print(f"\n{Fore.GREEN}All plots have been generated and saved in: {
          PLOT_CONFIG['output_folder']}{Style.RESET_ALL}")
